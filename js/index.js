...
toggleIcon.addEventListener("click", function () {
  ...
  faXmark.classList.toggle("visible");
});

window.addEventListener("click", function (event) {
  if (!toggleIcon.contains(event.target) && !navMenu.contains(event.target)) {
    navMenu.classList.remove("show");
    faBars.classList.remove("hidden");
    faXmark.classList.remove("visible");
  }
});
...
let autoSlider = setInterval(() => { next.click(); }, 8000);
...
function reloadSlider() {
  ...
  autoSlider = setInterval(() => { next.click(); }, 8000);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    userIcon.addEventListener('click', function(event) {
        ...
        dropdownUser.style.display = dropdownUser.style.display === 'flex' ? 'none' : 'flex';
    });
...
// Hàm cập nhật trạng thái active của phân trang và vô hiệu hóa nút
function updatePagination() {
    // Cập nhật các trang "active"
    document.querySelectorAll(".listpage li").forEach((li, index) => {
        li.classList.toggle("active", parseInt(li.textContent) === currentPage);
    });

    // Vô hiệu hóa nút trang trước (left arrow) nếu đang ở trang đầu tiên
    ...
    // Vô hiệu hóa nút trang sau (right arrow) nếu đang ở trang cuối cùng
